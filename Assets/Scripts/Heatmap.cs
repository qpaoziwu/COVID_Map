...
public class Heatmap : MonoBehaviour
{
    ...
    */
    ...
    {
        //Time.timeScale = .1f;
        buffer = new ComputeBuffer(140, HeatData.size); //We can change count later but for now 100 per buffer
        material.SetBuffer("_HeatData", buffer); //Sets it up on the shader
        //Reference = GetComponent<DistrictReference>();
        ...
        for (int i = 0; i < data.Length; i++)
        {
            ...
            data[i].position = Reference.DistList[i].Dist.transform.position;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < data.Length; i++)
        {
            ...
            data[i].position = Reference.DistList[i].Dist.transform.position;
            data[i].radius = storedData[i].radius * radiusRatio; //Small demo size
